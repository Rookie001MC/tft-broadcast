...
<Navigation
	layout="sidebar"
	class="preset-filled-surface-50-950 flex h-full w-56 shrink-0 flex-col border-r border-surface-200-800"
>
	<Navigation.Content class="flex-1 overflow-y-auto px-3 py-4">
		{#each navGroups as group (group.label)}
			<Navigation.Group>
				<Navigation.Label
					class="text-surface-400-600 mb-1 px-2 text-xs font-semibold tracking-wider uppercase"
				>
					{group.label}
				</Navigation.Label>
					...
					<Navigation.TriggerAnchor
						...
						class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors
							{active
								...
								: 'text-surface-700-300 hover:preset-tonal-primary'}"
